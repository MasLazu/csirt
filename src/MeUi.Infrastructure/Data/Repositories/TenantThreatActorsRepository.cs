...
public class TenantThreatActorsRepository : ITenantThreatActorsRepository
{
    private readonly string _connectionString;

    public TenantThreatActorsRepository(IConfiguration configuration)
    {
    ...
    public async Task<List<ActorProfileDto>> GetActorProfilesAsync(Guid tenantId, DateTime start, DateTime end, int limit = 30, CancellationToken cancellationToken = default)
    {
        string sql = @"
...
";
        ...
        IEnumerable<ActorProfileDto> result = await connection.QueryAsync<ActorProfileDto>(sql, new { start, end, tenantId, limit }, commandTimeout: 300);
        return result.AsList();
    }

    public async Task<List<ActorCountryDistributionDto>> GetActorDistributionByCountryAsync(Guid tenantId, DateTime start, DateTime end, int limit = 15, CancellationToken cancellationToken = default)
    {
        string sql = @"
...
SELECT ""Country"" as Country, ""UniqueActors"" as UniqueActors
FROM actor_countries
ORDER BY ""UniqueActors"" DESC
LIMIT @limit
";
        using IDbConnection connection = CreateConnection();
        IEnumerable<ActorCountryDistributionDto> result = await connection.QueryAsync<ActorCountryDistributionDto>(sql, new { start, end, tenantId, limit }, commandTimeout: 300);
        return result.AsList();
    }    
public async Task<List<ActorAsnDto>> GetActorAsnAsync(Guid tenantId, DateTime start, DateTime end, int limit = 15, CancellationToken cancellationToken = default)
    {
        string sql = @"
WITH actor_asn AS (
  SELECT 
...
";
        using IDbConnection connection = CreateConnection();
        IEnumerable<ActorAsnDto> result = await connection.QueryAsync<ActorAsnDto>(sql, new { start, end, tenantId, limit }, commandTimeout: 300);
        return result.AsList();
    }

    public async Task<List<ActorActivityTimelineDto>> GetTopActorsActivityTimelineAsync(Guid tenantId, DateTime start, DateTime end, TimeSpan interval, int limit = 10, CancellationToken cancellationToken = default)
    {
...
WITH top_actors AS (
  ...
  WHERE ""DeletedAt"" IS NULL 
    ...
    AND ""AsnRegistryId"" IN (SELECT ""AsnRegistryId"" FROM ""TenantAsnRegistries"" WHERE ""TenantId"" = @tenantId AND ""DeletedAt"" IS NULL)
...
SELECT 
  ...
  CAST(te.""SourceAddress"" as TEXT) as ActorIp,
  COUNT(*) as Activity
FROM ""ThreatEvents"" te
WHERE te.""DeletedAt"" IS NULL 
  AND te.""Timestamp"" BETWEEN @start AND @end
...
";
        using IDbConnection connection = CreateConnection();
        IEnumerable<ActorActivityTimelineDto> result = await connection.QueryAsync<ActorActivityTimelineDto>(sql, new { start, end, tenantId, limit }, commandTimeout: 300);
        return result.AsList();
    }

    public async Task<List<ActorTtpDto>> GetActorTtpAnalysisAsync(Guid tenantId, DateTime start, DateTime end, int limit = 25, CancellationToken cancellationToken = default)
    {
        string sql = @"
WITH actor_ttp AS (
  SELECT 
...
";
        using IDbConnection connection = CreateConnection();
        IEnumerable<ActorTtpDto> result = await connection.QueryAsync<ActorTtpDto>(sql, new { start, end, tenantId, limit }, commandTimeout: 300);
        return result.AsList();
    }   
 public async Task<List<ActorSimilarityDto>> GetActorSimilarityAsync(Guid tenantId, DateTime start, DateTime end, int limit = 25, CancellationToken cancellationToken = default)
    {
        string sql = @"
WITH actor_profiles AS (
  SELECT 
...
actor_similarity AS (
  SELECT 
    ...
    a1.attack_types as ""A1AttackTypes"",
    a2.attack_types as ""A2AttackTypes"",
    a1.ports_used as ""A1Ports"",
    a2.ports_used as ""A2Ports"",
    a1.malware_families as ""A1Malware"",
    a2.malware_families as ""A2Malware"",
    LEAST(a1.attack_types, a2.attack_types) as ""CommonAttackTypes"",
    LEAST(a1.ports_used, a2.ports_used) as ""CommonPorts"",
    LEAST(a1.malware_families, a2.malware_families) as ""CommonMalware"",
    (LEAST(a1.attack_types, a2.attack_types) + LEAST(a1.ports_used, a2.ports_used) + LEAST(a1.malware_families, a2.malware_families)) as ""SimilarityScore""
...
SELECT 
  ...
  ""A1AttackTypes"" as A1AttackTypes,
  ""A2AttackTypes"" as A2AttackTypes,
  ""A1Ports"" as A1Ports,
  ""A2Ports"" as A2Ports,
  ""A1Malware"" as A1Malware,
  ""A2Malware"" as A2Malware,
  ""CommonAttackTypes"" as CommonAttackTypes,
  ""CommonPorts"" as CommonPorts,
  ""CommonMalware"" as CommonMalware,
...
";
        using IDbConnection connection = CreateConnection();
        IEnumerable<ActorSimilarityDto> result = await connection.QueryAsync<ActorSimilarityDto>(sql, new { start, end, tenantId, limit }, commandTimeout: 300);
        return result.AsList();
    }

    public async Task<List<ActorPersistenceDto>> GetActorPersistenceAsync(Guid tenantId, DateTime start, DateTime end, CancellationToken cancellationToken = default)
    {
        string sql = @"
...
SELECT ""PersistenceType"" as PersistenceType, ""Actors"" as Actors
FROM persistence_categories
ORDER BY ""Actors"" DESC
";
        using IDbConnection connection = CreateConnection();
        IEnumerable<ActorPersistenceDto> result = await connection.QueryAsync<ActorPersistenceDto>(sql, new { start, end, tenantId }, commandTimeout: 300);
        return result.AsList();
    }

    public async Task<List<ActorEvolutionDto>> GetActorEvolutionAsync(Guid tenantId, DateTime start, DateTime end, int limit = 25, CancellationToken cancellationToken = default)
    {
        string sql = @"
...
";
        using IDbConnection connection = CreateConnection();
        IEnumerable<ActorEvolutionDto> result = await connection.QueryAsync<ActorEvolutionDto>(sql, new { start, end, tenantId, limit }, commandTimeout: 300);
        return result.AsList();
    }
}