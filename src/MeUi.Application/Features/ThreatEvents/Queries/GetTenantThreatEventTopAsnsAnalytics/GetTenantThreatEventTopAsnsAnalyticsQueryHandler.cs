...
public class GetTenantThreatEventTopAsnsAnalyticsQueryHandler : IRequestHandler<GetTenantThreatEventTopAsnsAnalyticsQuery, ThreatEventAsnTopAnalyticsDto>
{
    private readonly IThreatEventAnalyticsRepository _analyticsRepository;

    public GetTenantThreatEventTopAsnsAnalyticsQueryHandler(IThreatEventAnalyticsRepository analyticsRepository)
    {
        _analyticsRepository = analyticsRepository;
    }

    public async Task<ThreatEventAsnTopAnalyticsDto> Handle(GetTenantThreatEventTopAsnsAnalyticsQuery request, CancellationToken ct)
    {
        ...
        var asns = (await _analyticsRepository.GetAsnAnalyticsAsync(start, end, request.TopLimit, request.TenantId, ct)).ToList();
